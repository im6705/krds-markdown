---
title: "디지털 정부서비스 UI/UX 가이드라인 - KRDS 가이드 문서"
source: "디지털 정부서비스 UI/UX 가이드라인 (2025.08)"
total_pages: 1181
---

# KRDS 가이드라인 문서

이 문서는 **디지털 정부서비스 UI/UX 가이드라인 (2025.08)** PDF(1181페이지)의 핵심 내용을 추출하여 정리한 참조 문서이다.

---

## 목차

### 가이드라인 소개

| # | 파일 | 제목 | PDF 페이지 |
|---|---|---|---|
| 01 | [01-overview.md](./01-overview.md) | 가이드라인 소개, 구성, 활용방법, KRDS 소개 | 9-40 |
| 02 | [02-design-principles.md](./02-design-principles.md) | 디자인 원칙 7가지 | 49-62 |

### 스타일 (Style)

| # | 파일 | 제목 | PDF 페이지 |
|---|---|---|---|
| 03 | [03-color.md](./03-color.md) | 색상 시스템 (팔레트, 매직넘버, 시스템 색상, 적용 규칙) | 74-101 |
| 04 | [04-typography.md](./04-typography.md) | 타이포그래피 (서체, 스케일, 계층, 접근성) | 102-125 |
| 05 | [05-shape.md](./05-shape.md) | 형태/래디어스 (5단계, 계산법) | 126-138 |
| 06 | [06-layout.md](./06-layout.md) | 레이아웃 (그리드, 브레이크포인트, 간격) | 139-161 |
| 07 | [07-elevation.md](./07-elevation.md) | 엘리베이션 (6단계, 그림자, 딤드) | 177-191 |
| 08 | [08-high-contrast.md](./08-high-contrast.md) | 선명한 화면 모드 및 다크 모드 (디스플레이 모드, CSS/JS 구현, 모드별 토큰) | 192-204 |
| 09 | [09-design-tokens.md](./09-design-tokens.md) | 디자인 토큰 (3-level 체계, CSS/JS/HTML 구현, 빌드 파이프라인, 버전 관리, 마이크로프론트엔드) | 205-216 |

### 컴포넌트 (Components)

| # | 파일 | 제목 | PDF 페이지 |
|---|---|---|---|
| 10 | [10-components.md](./10-components.md) | 컴포넌트 개요 (10개 범주, 아이덴티티 상세) | 217-250 |

### 패턴 (Patterns)

| # | 파일 | 제목 | PDF 페이지 |
|---|---|---|---|
| 11 | [11-patterns-login.md](./11-patterns-login.md) | 로그인 서비스 패턴 (7단계 플로, HTML/CSS/JS 구현, 폼 검증, 세션 관리) | 977-1043 |
| 12 | [12-patterns-application.md](./12-patterns-application.md) | 신청 서비스 패턴 (4단계 플로) | 1052-1129 |
| 13 | [13-patterns-basic.md](./13-patterns-basic.md) | 기본 패턴 (개인정보입력, 도움, 동의, 목록탐색) | 754-798 |
| 14 | [14-application-levels.md](./14-application-levels.md) | 적용 수준 전략 (Do/Better/Best 동적 전환, 설정 관리, React/JS 구현, 컴플라이언스 점검) | 36-40 |

---

## 핵심 수치 참조표

### 브레이크포인트 (표준형 4단계)

| name | Viewport | 칼럼 수 | 가터 | 스크린 마진 |
|---|---|---|---|---|
| small | 360px~ | 4 | 16px | 16px |
| medium | 768px~ | 8 | 16px | 24px |
| large | 1024px~ | 12 | 24px | 24px |
| xlarge | 1280px~ | 12 | 24px | 24px |

### 매직넘버 (명도 대비)

| 매직넘버 | 대비 비율 | WCAG 기준 |
|---|---|---|
| 40 | 3:1 | AA (대형 텍스트/UI) |
| 50 | 4.5:1 | AA (일반 텍스트) |
| 70 | 7:1 | AAA |
| 90 | 15:1 | 최고 수준 |

### 래디어스 (표준형 5단계)

| Level | Radius | Container Size 범위 |
|---|---|---|
| Xsmall | 2px | 8~16px |
| Small | 4px | 20~32px |
| Medium | 6px | 40~64px |
| Large | 10px | 72~80px |
| Xlarge | 12px (max) | 96~120px |

### 타이포그래피 기본

| 항목 | 값 |
|---|---|
| 기본 서체 | Pretendard GOV |
| 기본 본문 크기 | 17px |
| 최소 본문 크기 | 16px |
| 글자 두께 | regular(400), bold(700) |
| 줄 간격 | 150% 이상 |
| rem 기본값 | 10px (62.5%) |

### 색상 비율

| 비율 | 역할 |
|---|---|
| 60% | 배경 및 중립적인 색상 |
| 30% | 보조 색상 |
| 10% | 주요 기능/상호작용 색상 |
| 최대 5% | 강조 색상 (accent) |

### 적용 수준 (서비스 패턴)

| 수준 | 설명 |
|---|---|
| 필수 (Do) | 미준수 시 작업 실패 |
| 권장 (Better) | 미준수 시 작업 난도 증가 |
| 우수 (Best) | 준수 시 만족도 기하급수적 증가 |

---

## 관련 법령

- 전자정부법
- 행정기관 및 공공기관 정보시스템 구축/운영지침
- 전자정부 웹사이트 품질관리 지침
- 한국형 웹 콘텐츠 접근성 지침 (KWCAG 2.2)
- WCAG 2.1 (적합도 수준 AA 목표)

## 저작권
본 마크다운 문서는 대한민국 디지털 정부 디자인 시스템(KRDS)의 이용약관을 따름.

[KRDS 저작권](https://www.krds.go.kr/html/site/utility/utility_06.html)
